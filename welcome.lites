...
@section('title') TAMARAW Marketplace @endsection
...
<header class="fixed top-0 inset-x-0 z-50 bg-cream/96 backdrop-blur-sm border-b border-ink/10">
  <div class="max-w-7xl mx-auto px-5 h-16 flex items-center justify-between gap-6">
    ...
    <nav class="hidden lg:flex items-center gap-7">
      @foreach(['Marketplace','For Farmers','AI Assistant','Cooperatives','Pricing'] as $link)
        <a href="#" class="text-sm text-muted-fg hover:text-ink transition-colors">{{ $link }}</a>
      @endforeach
    </nav>
  ...
  <div id="mobile-nav" class="hidden lg:hidden bg-cream border-t border-ink/10 px-5 py-5 flex flex-col gap-4">
    @foreach(['Marketplace','For Farmers','AI Assistant','Cooperatives','Pricing'] as $link)
      <a href="#" class="text-sm text-ink">{{ $link }}</a>
    @endforeach
  </div>
</header>
...
<section class="pt-16 min-h-screen grid grid-cols-1 lg:grid-cols-2">
  <div class="flex flex-col justify-center px-8 md:px-14 lg:px-20 py-20 lg:py-28">
    ...
    <div class="mt-14 pt-8 border-t border-ink/10 grid grid-cols-4 gap-4">
      @foreach([['240+','Registered Farms'],['18','Barangays Served'],['₱4.2M','Sales Facilitated'],['1,200+','Active Buyers']] as $stat)
        <div>
          <div class="font-display text-2xl font-light text-ink">{{ $stat[0] }}</div>
          <div class="text-[11px] text-muted-fg mt-1 leading-snug">{{ $stat[1] }}</div>
        </div>
      @endforeach
    </div>
  </div>
  ...
  <div class="flex gap-10 whitespace-nowrap ticker-scroll">
    @foreach([0,1,2,3] as $i)
      <span class="flex items-center gap-10 text-sm shrink-0">
        @foreach(['· Ampalaya ₱45/kg','· Pechay Baguio ₱60/kg','· Kamatis ₱75/kg','· Sitaw ₱55/kg','· Saging na Saba ₱35/kg','· Palay ₱22/kg','· Talong ₱50/kg','· Kangkong ₱40/kg'] as $item)
          <span>{{ $item }}</span>
        @endforeach
      </span>
    @endforeach
...
<section id="marketplace" class="max-w-7xl mx-auto px-6 py-20 md:py-28">
  <div class="flex flex-col md:flex-row md:items-end justify-between gap-6 mb-12">
    ...
    <div class="flex flex-wrap gap-2" id="category-filters">
      @foreach(['All','Vegetables','Fruits','Grains','Livestock','Processed'] as $cat)
        <button
          data-cat="{{ $cat }}"
          class="cat-btn px-4 py-1.5 text-xs font-semibold border transition-all duration-200 {{ $cat === 'All' ? 'bg-forest text-cream border-forest' : 'bg-transparent text-muted-fg border-ink/20 hover:border-forest/50 hover:text-ink' }}"
        >{{ $cat }}</button>
      @endforeach
    </div>
  </div>
  ...
  <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-px bg-ink/10" id="product-grid">
    ...
    @foreach($listings as $product)
      <div class="product-card group bg-card flex flex-col overflow-hidden" data-category="{{ $product['category'] }}">
        <div class="relative overflow-hidden bg-muted" style="aspect-ratio:4/3">
          ...
          />
          @if($product['badge'])
            <span class="absolute top-3 left-3 bg-amber text-ink text-[10px] font-bold tracking-wider uppercase px-2.5 py-1">{{ $product['badge'] }}</span>
          @endif
        ...
        <div class="p-5 flex flex-col flex-1">
          <div class="flex items-start justify-between gap-2 mb-1">
            <p class="text-[10px] font-bold tracking-[0.15em] uppercase text-muted-fg">{{ $product['category'] }}</p>
            <p class="text-[10px] text-muted-fg">Stock: {{ $product['stock'] }} kg</p>
          </div>
          <h3 class="font-display font-light text-ink text-lg mb-1">{{ $product['name'] }}</h3>
          <p class="text-xs text-muted-fg mb-1">{{ $product['farm'] }}</p>
          <p class="text-[11px] text-muted-fg/70 mb-4">{{ $product['barangay'] }}</p>

          <div class="mt-auto pt-4 border-t border-ink/10">
            <div class="flex items-center justify-between mb-3">
              <div class="flex items-baseline gap-1">
                <span class="font-display text-2xl text-ink">₱{{ $product['price'] }}</span>
                <span class="text-xs text-muted-fg">{{ $product['unit'] }}</span>
              </div>
              <span class="text-[11px] text-muted-fg border border-ink/15 px-2 py-0.5">Min. {{ $product['minOrder'] }}</span>
            </div>
  ...
  <div class="mt-10 text-center">
    <a href="#" class="inline-flex items-center gap-2 text-sm text-ink border border-ink/25 px-8 py-3 hover:border-forest hover:text-forest transition-colors">
      View All Products
      <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M5 12h14M12 5l7 7-7 7"/></svg>
    </a>
  </div>
...
<section id="ai" class="bg-ink text-cream py-20 px-6">
  <div class="max-w-7xl mx-auto">
    ...
    <div class="flex flex-wrap gap-2 mb-8">
      @foreach([['crop','🌱','Crop Advisor'],['market','📊','Market Prices'],['weather','🌤','Weather & Alerts'],['calendar','📅','Planting Calendar']] as $tab)
        <button
          data-ai="{{ $tab[0] }}"
          class="ai-tab flex items-center gap-2 px-4 py-2.5 text-sm font-medium border transition-all duration-200 {{ $tab[0] === 'crop' ? 'bg-gold text-ink border-gold' : 'bg-transparent text-cream/60 border-cream/15 hover:border-cream/40 hover:text-cream' }}"
        >
          <span>{{ $tab[1] }}</span>
          <span>{{ $tab[2] }}</span>
        </button>
    ...
    <div class="grid grid-cols-1 lg:grid-cols-2 gap-8 items-start">
      ...
      <div id="ai-panel">
        ...
        <div id="panel-crop" class="ai-panel-content bg-ai-light border border-ai/15 p-5 space-y-3">
          ...
          @foreach([['Pechay Baguio','High demand in Calapan market · Low competition · 45-day harvest','+₱18,400 projected'],['Sitaw','Favorable rainfall forecast · Matches your soil profile','+₱12,200 projected'],['Ampalaya','Peak pricing season (Jul–Sep) · Fast turnover','+₱9,600 projected']] as $item)
            <div class="bg-white border border-ai/10 p-4 flex items-start justify-between gap-4">
              <div>
                <p class="font-semibold text-sm text-ink">{{ $item[0] }}</p>
                <p class="text-xs text-muted-fg mt-0.5 leading-relaxed">{{ $item[1] }}</p>
              </div>
              <span class="text-xs font-bold text-forest whitespace-nowrap bg-forest/10 px-2 py-1">{{ $item[2] }}</span>
            </div>
          @endforeach
        </div>
        ...
        <div id="panel-market" class="ai-panel-content hidden bg-ai-light border border-ai/15 p-5">
          ...
          @foreach([['Kamatis',75,68,'up'],['Ampalaya',45,50,'down'],['Pechay',60,58,'up'],['Sitaw',55,55,'flat'],['Talong',50,44,'up']] as $row)
            <div class="grid grid-cols-4 gap-2 py-2.5 border-b border-ink/6 text-sm">
              <span class="font-medium text-ink">{{ $row[0] }}</span>
              <span class="text-right font-semibold text-ink">₱{{ $row[1] }}</span>
              <span class="text-right text-muted-fg">₱{{ $row[2] }}</span>
              <span class="text-right font-bold {{ $row[3] === 'up' ? 'text-forest' : ($row[3] === 'down' ? 'text-sienna' : 'text-muted-fg') }}">
                {{ $row[3] === 'up' ? '↑' : ($row[3] === 'down' ? '↓' : '→') }}
              </span>
            </div>
          @endforeach
        ...
        <div id="panel-weather" class="ai-panel-content hidden bg-ai-light border border-ai/15 p-5 space-y-3">
          ...
          @foreach([
            ...
            @endphp
            <div class="border p-4 {{ $levelClass }}">
              <div class="flex items-center justify-between mb-1">
                <span class="text-[10px] font-bold tracking-widest">{{ $levelLabel }}</span>
                <span class="text-[10px] opacity-60">{{ $alert[2] }}</span>
              </div>
              <p class="text-xs leading-relaxed text-ink">{{ $alert[1] }}</p>
            </div>
          @endforeach
        </div>
        ...
        <div id="panel-calendar" class="ai-panel-content hidden bg-ai-light border border-ai/15 p-5">
          ...
          <div class="grid grid-cols-2 gap-3">
            @foreach([['Jul',['Pechay','Kangkong'],'Plant now'],['Aug',['Sitaw','Ampalaya'],'Prepare beds'],['Sep',['Kamatis','Talong'],'Schedule seedling'],['Oct',['Kamote','Gabi'],'Upcoming']] as $row)
              @php
              ...
              <div class="bg-white border border-ink/10 p-4">
                <div class="flex items-center justify-between mb-3">
                  <span class="font-display text-2xl font-light text-ink">{{ $row[0] }}</span>
                  <span class="text-[10px] font-bold px-2 py-0.5 {{ $statusClass }}">{{ $row[2] }}</span>
                </div>
                <div class="flex flex-wrap gap-1">
                  @foreach($row[1] as $crop)
                    <span class="text-xs bg-forest/8 text-forest px-2 py-0.5">{{ $crop }}</span>
                  @endforeach
                </div>
              </div>
            @endforeach
      ...
      <div class="space-y-8">
        @foreach([
          ...
          <div class="flex gap-5">
            <span class="font-display text-3xl font-light text-muted shrink-0 mt-0.5">{{ $step[0] }}</span>
            <div>
              <h4 class="font-semibold text-ink mb-1">{{ $step[1] }}</h4>
              <p class="text-sm text-muted-fg leading-relaxed">{{ $step[2] }}</p>
            </div>
          </div>
        @endforeach
      </div>
...
{{-- ── TESTIMONIALS ─────────────────────────────────────────────── --}}
<section class="bg-muted py-20 px-6">
  <div class="max-w-7xl mx-auto">
      ...
      @foreach([
        ...
        <blockquote class="bg-cream p-7 flex flex-col">
          <div class="flex items-center gap-3 mb-5">
            <img src="{{ $t[4] }}" alt="{{ $t[2] }}" class="w-10 h-10 object-cover"/>
            <div>
              <p class="font-semibold text-sm text-ink">{{ $t[2] }}</p>
              <p class="text-xs text-muted-fg">{{ $t[3] }}</p>
            </div>
          </div>
          <p class="text-sm text-ink leading-relaxed flex-1 mb-5 italic">{{ $t[1] }}</p>
          <span class="text-xs font-bold text-forest bg-forest/10 px-2.5 py-1 w-fit">{{ $t[5] }}</span>
        </blockquote>
      @endforeach
...
<section class="max-w-7xl mx-auto px-6 py-20 md:py-28">
  ...
  <div class="grid grid-cols-1 md:grid-cols-3 gap-px bg-ink/10">
    ...
    @foreach([
      ...
      <div class="bg-card p-8 flex flex-col {{ $plan[4] ? 'ring-2 ring-forest relative' : '' }}">
        @if($plan[4])
        ...
        @endif
        <p class="text-xs font-bold tracking-widest uppercase text-muted-fg mb-1">{{ $plan[1] }}</p>
        <h3 class="font-display text-xl font-semibold text-ink mb-4">{{ $plan[0] }}</h3>
        <div class="flex items-baseline gap-1.5 mb-6 pb-6 border-b border-ink/10">
          ...
          @else
            <span class="font-display text-4xl font-light">₱{{ $plan[2] }}</span>
            <span class="text-xs text-muted-fg">{{ $plan[3] }}</span>
          @endif
        </div>
        ...
        <a href="#" class="text-sm font-medium text-center px-6 py-3 transition-colors {{ $plan[4] ? 'bg-forest text-cream hover:bg-forest/90' : 'border border-ink/25 text-ink hover:border-forest hover:text-forest' }}">
          {{ $plan[5] }}
        </a>
      </div>
    @endforeach
...
<footer class="bg-ink text-cream/50 py-14 px-6">
  <div class="max-w-7xl mx-auto grid grid-cols-1 md:grid-cols-5 gap-10">
    ...
    @foreach([
      ...
      <div>
        <h4 class="text-[10px] font-bold tracking-[0.22em] uppercase text-cream mb-4">{{ $col[0] }}</h4>
        <ul class="space-y-2.5">
          @foreach($col[1] as $link)
            <li><a href="#" class="text-xs hover:text-cream transition-colors">{{ $link }}</a></li>
          @endforeach
        </ul>
      </div>
    @endforeach
